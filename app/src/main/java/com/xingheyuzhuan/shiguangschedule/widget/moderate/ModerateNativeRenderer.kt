    ...
    fun render(context: Context, snapshot: WidgetSnapshot): RemoteViews {
        ...
        displayCourses.forEachIndexed { i, course ->
            ...
            if (course.color_int < style.course_color_maps.size) {
                val colors = style.course_color_maps[course.color_int]
                itemRv.setInt(R.id.course_indicator, "setBackgroundColor", colors.light_color.toInt())
                itemRv.setInt(R.id.course_indicator_dark, "setBackgroundColor", colors.dark_color.toInt())
            }
